\begin{table} 
\footnotesize
\iflatexml\else
\resizebox{\linewidth}{!}{
\fi
    \begin{tabular}{ l p{3.5in} c c c  }
    Name & URL & License & Activity & Citation \\ \hline
CCP1GUI	& \url{http://www.scd.stfc.ac.uk/research/app/40501.aspx}  & GPL & C3 & \\
ccwatcher & \url{http://sourceforge.net/projects/ccwatcher}  & GPL & B2 & \\
Gabedit & \url{http://gabedit.sourceforge.net} & BSD &  C1 & \cite{Allouche_2010} \\
J-ICE & \url{http://j-ice.sourceforge.net} & GPL & A1 & \cite{Canepa_2010} \\
QMForge	& \url{http://qmforge.sourceforge.net}  & GPL & A1 & \\
 wxMacMolPlt & \url{http://brettbode.github.io/wxmacmolplt} & GPL & A1 & \cite{Bode_1998} \\
    \end{tabular} 
\iflatexml\else
}
\fi
    \caption{\label{qmviz} Open source software for quantum chemistry visualization.}
\end{table}
